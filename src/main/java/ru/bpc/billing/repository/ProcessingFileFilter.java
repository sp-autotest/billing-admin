 ...
 * Time: 15:26
 */
public class ProcessingFileFilter implements Specification<ProcessingFile> {
    ...
    public ProcessingFileFilter(String filename, Date fromCreateDate, Date toCreateDate) {
    ...
    @Override
    public Predicate toPredicate(Root<ProcessingFile> root, CriteriaQuery<?> criteriaQuery, CriteriaBuilder criteriaBuilder) {
        if ( null != id ) {
        ...
        if ( null != getProcessingDate() || null != getBillingFileFormat() || null != getCountLines() || null != getCarrierId()) {
            ...
            if (null != getCarrierId()){
                predicates.add(criteriaBuilder.equal(billingFileRoot.<Long>get("carrier"), getCarrierId()));
            }
        }
